  ...
  for (const dir of directories) {
    ...
    for (const file of files) {
      ...
      fileData.push({
        ...
        width: partsOfFileName[1]?.replace('w', ''), // e.g. 'w800' -> '800'
        lvl: (partsOfFileName[2] ?? '').startsWith('lvl')
          ? partsOfFileName[2].slice(3, 4)
          : '-',
        q: (partsOfFileName[2] ?? '').startsWith('q')
          ? partsOfFileName[2].slice(1)
          : '-',
        compression: file.includes('lossy') ? 'STRATNA' : 'BEZSTRATNA',
        fileType: file.split('.')[1],
        interlaced:
          file.includes('progressive') || file.includes('interlaced')
            ? 'TAK'
            : 'NIE',
