...
const generateAvifLosslessUrl = (img: number, width: number, q_l: number) =>
  `${BUCKET_URL}/AVIF_LOSSLESS/${img}_w${width}_lvl${q_l}_lossless.avif`;

const generateAvifLossyUrl = (img: number, width: number, q_l: number) =>
  `${BUCKET_URL}/AVIF_LOSSY/${img}_w${width}_q${q_l}_lossy.avif`;

const generateWebpLosslessUrl = (img: number, width: number, q_l: number) =>
  `${BUCKET_URL}/WEBP_LOSSLESS/${img}_w${width}_q${q_l}_lossless.webp`;

const generateWebpLossyUrl = (img: number, width: number, q_l: number) =>
...
const generateReferenceFileUrl = (img: number, width: number) =>
